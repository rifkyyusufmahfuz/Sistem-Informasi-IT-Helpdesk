...
@section('contents')
            ...
            <div class="col-xl-12">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">Status Otorisasi</h6>
                    </div>
                    <!-- Card Body -->
                    <!-- View -->
                    <div class="card-body">
                        <div class="chart-pie">
                            @if (!empty($sortedChartDataOtorisasi))
                                <canvas id="otorisasi-chart"></canvas>
                            @else
                                <p class="text-center">Belum ada otorisasi</p>
                            @endif
                        </div>
                        <div class="mt-2 text-center small">
                            ...
                            <span class="mr-2 small">
                                <i class="fas fa-circle text-warning"></i> Menunggu Otorisasi
                            </span>
                            <span class="mr-2 small">
                                <i class="fas fa-circle text-secondary"></i> Revisi
                            </span>
                            <span class="mr-2 small">
    ...
    <script>
        // Cek apakah data otorisasi ada atau tidak
        var sortedChartDataOtorisasi = {!! json_encode($sortedChartDataOtorisasi) !!};
        var isDataAvailableOtorisasi = Object.values(sortedChartDataOtorisasi).some(val => val > 0);

        // Tampilkan grafik batang jika data ada
        if (isDataAvailableOtorisasi) {
            var otorisasiLabels = ['Belum diajukan', 'Menunggu Otorisasi', 'Disetujui', 'Revisi', 'Ditolak'];
            var jumlahOtorisasi = Object.values(sortedChartDataOtorisasi);

            var otorisasiChartCanvas = document.getElementById('otorisasi-chart').getContext('2d');
            var otorisasiChart = new Chart(otorisasiChartCanvas, {
                type: 'pie',
                data: {
                    labels: otorisasiLabels,
                    datasets: [{
                        label: 'Jumlah Otorisasi',
                        data: jumlahOtorisasi,
                        backgroundColor: [
                            '#e74a3b', '#f6c23e', '#1cc88a', '#858796', '#5a5c69'
                        ] // Sesuaikan warna sesuai dengan jumlah status otorisasi yang ada
                    }]
                },
                options: {
                    maintainAspectRatio: false,
